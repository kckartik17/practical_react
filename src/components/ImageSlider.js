...
export default class ImageSlider extends React.Component {
  ...
  nextImage = () => {
    this.setState((state) => ({
      ...state,
      idx: state.idx + 1 > state.data.images.length - 1 ? 0 : state.idx + 1,
    }));
  };
  ...
  previousImage = () => {
    this.setState((state) => ({
      ...state,
      idx: state.idx - 1 < 0 ? state.data.images.length - 1 : state.idx - 1,
    }));
  };
    ...
    return (
      <div>
        ...
        <img
          ...
          alt="imge"
          src={this.state.data.images[this.state.idx]}
        />
